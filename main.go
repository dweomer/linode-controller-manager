//go:generate go tool -modfile=tool/controller-gen/go.mod controller-gen crd object rbac:roleName=controller-manager,fileName=controller-manager.yaml webhook paths=./... output:crd:artifacts:config=./manifest/crd output:rbac:dir=./manifest/rbac
package main
